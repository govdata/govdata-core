...
from System.Gmail import Gmail
import os, time, sys, numpy, inspect, re, subprocess, traceback
import cPickle as pickle
from System.Utils import *
from System.LinkManagement import *
from System.MetaData import MakeRuntimeMetaData
try:
	from System.config.SetupFunctions import DE_NAME
except:
	print 'DE_NAME not imported from SetupFunctions, using defulat.'
	traceback.print_exc()
	DE_NAME = 'Data Environment'
try:
	from System.config.PerMachineSetup import DEFAULT_CALLMODE
except:
	print 'DE_NAME not imported from SetupFunctions, using defulat.'
	traceback.print_exc()
...
RUNSTDOUT = 'MostRecentRun'	
RUNSTDOUTINSESSION = RUNSTDOUT + ".InSession"
RUNTEMPOUTPUT = 'RUNTEMP'
TEMPCMDFILE = 'CMDTEMP'
TEMPMETAFILE='METATEMP'
SsIDFile = TEMPFOLDER + '/SsID.txt'


def SetupRun(creates = ('../Archive/','../System/Tmp/')):
...
def MakeUpdated(Targets,Exceptions = None,depends_on = ('../',), Simple = True, Forced = False,Pruning=True,ProtectComputed = False,EmailWhenDone=None,CallMode=DEFAULT_CALLMODE):	
	'''
...
def LinkUpdate(Seed,AU = None, Exceptions = None, depends_on=('../',),creates=('../System/',),Simple=False,Forced = False,Pruning = True,ProtectComputed = False,EmailWhenDone = None,CallMode=DEFAULT_CALLMODE):	
	'''
...
def UpdateLinks(ActivatedLinkListSequence, Seed, AU = None, Exceptions = None, Simple=None,Pruning=None,Forced=None,ProtectComputed = False, depends_on=('../',),creates=('../System',),EmailWhenDone = None,CallMode=DEFAULT_CALLMODE):	
...
def DoOp(i,j,SsName,SsTemp,SsRTStore,CreatesList,IsFast,CallMode,TouchList,DepListj, EmailWhenDone,creates=('../',)):
	...
	pickle.dump({'NCS':NewlyCreatedScripts,'OriginalTimes':OriginalTimes,'ExitType':ExitType,'IsDifferent':IsDifferent},F)
	F.close()
...
def CheckDiffs(j,Creates,SsRTStore,IsFast,RunOutput,IsDifferent,Targets):
	...
	for f in Creates:
		...
		if PathExists(temp_name):
			...
			archive_name = '../Archive/Archive_' + TS + '_' + f[3:].replace('/','__')
...
def GetPythonPath():
	...
	try:
		from System.config.PerMachineSetup import PATH_TO_PYTHON
	except:
		print 'PATH_TO_PYTHON not imported from PerMachineSetup, using default'
		traceback.print_exc()
...
def SetupOp(j,CreateList,SsTemp,creates=('../System/Tmp/',)):
	OriginalTimes = {}
	OriginalDirInfo = {}	
...
def EmailResults(EmailWhenDone,SsName,FileName):
	...
	if EmailList:
		try:
			Gmail(account,password,EmailList,subject = DE_NAME + ' Run ID#: ' + SsName, FileName=FileName)
		except:
			print 'No email sent.'
			traceback.print_exc()
