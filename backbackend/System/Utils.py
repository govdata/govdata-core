...
def activate(x,y,*z):
	return lambda f : activator(f,x,y,*z)

def activator(f,l1,l2,*l3):
	if l1 != None:
		f.__dependor__ = l1
	if l2 != None:
		f.__creator__ = l2
...
def is_file_name(name):
	return is_string_like(name) and os.path.split(name)[1] == name
...
def MaximalCommonPath(PathList):
	...
	if len(PathList) > 0:
		PathList = [p[:-1] if len(p) > 0 and p[-1] ==  '/' else p for p in PathList]
...
def DictInvert1(D, order=None):
	'''
		ARGUMENT:
			dictionary D, the value corresponding to each key in D is a list
		OUTPUT:
		--dictionary whose keys are unique elements of values of D, and 
		whose values on key 'K' are lists of keys 'k' in D such that K is in D[k]
	'''
	if order is None:
		order = D.keys()
	return dict([(v,uniqify([j for j in order if v in D[j]])) for v in uniqify(ListUnion(D.values()))])
