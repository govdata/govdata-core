...
def MakePyRepresentation(inpath,outpath):
	s = '<a href="/System/CGI-Executables/RepresentationMaker.cgi?Representation=' + inpath +'&OpenFile=Yes">Open File</a>'
	convert_files([inpath],[outpath],tblfile=None,openwithline=s)

def modulelink(module, baseurl=''):
    """Hyperlink to a module, either locally or on python.org"""
    if module+'.py' not in local_files:
        baseurl = 'http://www.python.org/doc/2.5.1/lib/module-'
...
def convert_files(filenames, outpathnames, local_filenames=None, tblfile='readme.htm',openwithline=None):
	global local_files
	local_files = local_filenames or filenames
	summary_table = {}
	for (f,o) in zip(filenames,outpathnames):
		fulltext = '\n'.join(map(string.rstrip, open(f,'r').readlines())).replace("'''",'"""')
		text = fulltext
		for (pattern, repl) in replacements:
			text = re.sub(pattern, repl, text)
		text = '<<header("AIMA Python file: %s")>><pre>%s</pre><<footer>>' % (f, text)
		if openwithline is not None:
			text = openwithline + '\n' + text
		open(o, 'w').write(text)
		if tblfile:
