...
from System.Utils import MakeDir, Contents, listdir, IsDir, wget, uniqify, PathExists,RecursiveFileList, ListUnion
...
def NEA_preparser2(inpath,outpath,L = None):

	filepath = outpath + '__PARSE__/'
	MakeDir(filepath)
...
def NIPA_downloader(download_dir):

	MakeDir(download_dir)
   
	get_manifest(download_dir)
	...
	X = tb.tabarray(SVfile = download_dir + 'manifest.tsv')
	for x in X:
		...
		topath = download_dir + '__PARSE__/' + x['Number'].strip('.') + '_' + Freq + '.csv'
		
		WgetMultiple(url,topath)


@activate(lambda x : tuple(x[1].values()), lambda x : x[0]) 
def NIPA_preparser1(targetdir,sourcedirs):
	MakeDir(targetdir)
	
	MakeDir(targetdir + '__PARSE__')
	source = sourcedirs['raw'] 
	M = tb.tabarray(SVfile = source + 'manifest.tsv')
	for x in M:
		f = source + '__PARSE__/' + x['Number'].strip('.') + '_' + x['Freq'] + '.csv'
		print f 
		savepath = targetdir + '__PARSE__/' + x['Number'].strip('.') + '_' + x['Freq'] + '.tsv'
...
@activate(lambda x : tuple(x[1].values()), lambda x : x[0]) 
def NIPA_preparser2(targetdir,sourcedirs):
	#get helper data 
	inpath = sourcedirs['preparse1']
	MakeDir(targetdir)
	NEA_preparser2(inpath + '__PARSE__/',targetdir)
	
	get_additional_data(targetdir)
	

def get_additional_data(download_dir,depends_on = 'http://www.bea.gov/national/nipaweb/Index.asp'):
	wget(depends_on,download_dir + 'additional_info.html')
	...
	X.saveSV(download_dir + 'addtional_info.csv',metadata=True)
...
def backend_BEA_NIPA(creates = OG.CERT_PROTOCOL_ROOT + NIPA_NAME + '.py'):
    OG.incrementalBackendProtocol2(NIPA_NAME,downloader = [(NIPA_downloader,'raw'),(NIPA_preparser1,'preparse1'),(NIPA_preparser2,'preparse2')],parser = OG.csv_parser,uptostep='updateCollection',incremental=True)
